...
if (isAliveP5) then {_task1 = [east, (str taskID), [_description, _title, _waypoint], [p5, true], "CREATED", 0, false, "", true] call BIS_fnc_taskCreate}, [str taskID, p5TaskIcon] call BIS_fnc_taskSetType, [str taskID, true] call BIS_fnc_taskSetAlwaysVisible; // Erstellt Task auf Unit, wenn diese nicht bewusstlos ist
...
if (isAliveP6) then {_task1 = [east, (str taskID), [_description, _title, _waypoint], [p6, true], "CREATED", 0, false, "", true] call BIS_fnc_taskCreate}, [str taskID, p6TaskIcon] call BIS_fnc_taskSetType, [str taskID, true] call BIS_fnc_taskSetAlwaysVisible;
westTaskID2 = taskID;
taskID = taskID + 1;
if (isAliveP7) then {_task1 = [east, (str taskID), [_description, _title, _waypoint], [p7, true], "CREATED", 0, false, "", true] call BIS_fnc_taskCreate}, [str taskID, p7TaskIcon] call BIS_fnc_taskSetType, [str taskID, true] call BIS_fnc_taskSetAlwaysVisible;
westTaskID3 = taskID;
taskID = taskID + 1;
if (isAliveP8) then {_task1 = [east, (str taskID), [_description, _title, _waypoint], [p8, true], "CREATED", 0, false, "", true] call BIS_fnc_taskCreate}, [str taskID, p8TaskIcon] call BIS_fnc_taskSetType, [str taskID, true] call BIS_fnc_taskSetAlwaysVisible;
westTaskID4 = taskID;
taskID = taskID + 1;

while {_loop != doubleWatchtowerMarkingTime} do {// Für 30 Sekunden wird diese while-Funktion 60 Mal ausgewertet, um Tasks zu löschen, falls Units sterben und um Tasks hinzuzufügen, falls Units wiederbelebt werden. 
	isAliveP5 = ((lifeState p5) isEqualTo "HEALTHY" or (lifeState p5) isEqualTo "INJURED"); // "true" wenn Unit lebt.
